...
const NotificationsMenu = () => {
  ...
  const getPushNotifications = async () => {
    ...
    const response = await pushNotificationsApi(userId).catch((err) =>
      console.log({ err })
    );
        ...
        {notificationsData.length > 0 ? (
          <div>
            {notificationsData.map((notification, index) => {
              if (
                notification.type === 'APPT_RESCHEDULE_BY_DOCTOR' &&
                notification.data.appointmentDetails
              ) {
                return (
                  <div key={index}>
              ...
              if (
                notification.type === 'APPT_CANCELLED_BY_DOCTOR' ||
                notification.type === 'APPT_CANCELLED_BY_PATIENT'
              ) {
                return (
                  <div key={index}>
